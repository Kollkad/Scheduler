...
Основные компоненты:
- Класс LawsuitChartAnalyzer: анализ данных для построения диаграмм
- Функции определения этапов дел: assign_lawsuit_stages, calculate_monitoring_status
- Функция построения таблицы результатов: build_new_table
- Эндпоинты API: /analyze_lawsuit_charts, /analyze_lawsuit, /filtered-cases
"""
...
class LawsuitChartAnalyzer:
    ...
    def analyze_for_charts(self) -> list:
        ...
        stats = {}
        for stage, checks in LAWSUIT_CHECKS_MAPPING.items():
            for check in checks:
        ...
        for _, row in self.staged_df.iterrows():
            ...
            if stage == "exceptions":
                # Для исключений status_str содержит тип исключения
                if status_str in stats.get("exceptionStatus", {}):
                    stats["exceptionStatus"][status_str] += 1
                continue

            # Стандартная обработка для остальных этапов
            if stage in LAWSUIT_CHECKS_MAPPING:
                checks = LAWSUIT_CHECKS_MAPPING[stage]
        ...
        for check_name, counts in stats.items():
            # Для проверки exception_status используются типы исключений как значения
