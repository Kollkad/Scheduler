...
Либо двойным щелчком, либо консольный запуск:

**Windows**:

```bash
project_scripts\run_project.bat
```

**Linux**:

```bash
./project_scripts/run_project.sh
```
...
**ProjectConverter.py** - утилита для конвертации файлов проекта в текстовый формат и обратно. Сохраняет структуру папок и расширения файлов, добавляя их в названия. Запуск: `python ProjectConverter.py` с выбором режима через консоль.
