...
set "DOWNLOAD_URL=https://token:%SBEROSC_TOKEN%@%SBEROSC_URL%/repo/extras/github api/%GITHUB_ORG%/%GITHUB_PROJECT%/releases/download/%GITHUB_TAG%/%GITHUB_FILE%"
...
if errorlevel 1 (
    echo Ошибка скачивания архива! Проверьте переменные и доступ к SBEROSC.
    pause
    exit /b 1
)
...
xcopy "%EXTRACTED%\*" "%CODE_DIR%\" /E /I /Y /Q >nul
...
if errorlevel 1 (
    ...
    echo   - В PYTHON_PATH указан неверный путь
    echo   - Если PYTHON_PATH пустой, то команда "python" должна быть в PATH
    echo.
    pause
    exit /b 1
)
...
if exist "%CODE_DIR%\backend\requirements.txt" (
    ...
    :: Устанавливаются зависимости (флаг --trusted-host для обхода SSL, при необходимости --user)
...
echo start /B "" "%VENV_DIR%\Scripts\pythonw.exe" app\main.py
